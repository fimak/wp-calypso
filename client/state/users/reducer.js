...
export const requesting = createReducer( {}, {
	[ USER_REQUEST ]: ( state, { user } ) => user ? { ...state, [ user.ID ]: true } : state,
	[ USER_REQUEST_SUCCESS ]: ( state, { user } ) => ( { ...state, [ user.ID ]: false } ),
	[ USER_REQUEST_FAILURE ]: ( state, { user } ) => ( { ...state, [ user.ID ]: false } )
} );
